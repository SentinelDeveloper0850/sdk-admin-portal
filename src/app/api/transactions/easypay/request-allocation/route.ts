...
import "@/server/actions/easypay-transactions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const easypayNumber = formData.get("easypayNumber");

    if (!transactionId || !policyNumber || !easypayNumber) {
      return NextResponse.json(
        {
          success: false,
    ...
    const policy = await AssitPolicyModel.findOne({
      membershipID: policyNumber,
    });
    if (!policy) {
      return NextResponse.json(
        {
    ...
    const existingAllocation = await AllocationRequestModel.findOne({
      ...
      status: {
        $nin: [
          EAllocationRequestStatus.REJECTED,
          EAllocationRequestStatus.CANCELLED,
        ],
      },
    });
