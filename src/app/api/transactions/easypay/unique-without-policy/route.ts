...
export async function GET(request: Request) {
  try {
    ...
    const formattedData = data.map((item: any) => ({
      ...
      lastTransactionDate:
        item.transactions[item.transactions.length - 1]?.date,
    ...
    console.error(
      "Error fetching unique EasyPay numbers without policy:",
      error.message
    );
    return NextResponse.json(
      {
        message:
          "Internal Server Error ~ Error fetching unique EasyPay numbers without policy",
      },
      { status: 500 }
    );
  }
