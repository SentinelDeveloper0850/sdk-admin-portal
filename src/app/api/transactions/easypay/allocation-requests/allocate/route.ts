    ...
    try {
        ...
        if (webhookUrl) {
            const payload = createSystemNotification(
                "Easypay Allocation Requests Allocated",
                `${res.modifiedCount} easypay request(s) have been marked as allocated by ${user.name || user.email}.\ncc: @wethusdk`,
                "success"
            );
            ...
            await sendDiscordNotification(webhookUrl, payload, "easypay-allocation-allocated");
        }

        return NextResponse.json({ updated: res.modifiedCount, matched: res.matchedCount });
    } catch (error) {
        console.error("Error allocating easypay requests:", (error as Error).message);
        return NextResponse.json(
            { message: "Internal Server Error ~ allocate easypay requests" },
            { status: 500 }
        );
    }
}
