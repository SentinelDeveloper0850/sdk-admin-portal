...
export async function POST(request: NextRequest) {
    try {
        ...
        const res = await EasypayAllocationRequestModel.updateMany(
            ...
            { $set: { status: EAllocationRequestStatus.DUPLICATE, markedAsDuplicateBy: user._id, markedAsDuplicateAt: new Date() } }
        );
        ...
        if (webhookUrl) {
            const payload = createSystemNotification(
                "Easypay Allocation Requests Marked as Duplicates",
                `${res.modifiedCount} easypay request(s) have been marked as duplicates by ${user.name || user.email}.\ncc: @wethusdk`,
                "warning"
            );
            ...
            await sendDiscordNotification(webhookUrl, payload, "easypay-allocation-duplicates");
        }

        return NextResponse.json({ updated: res.modifiedCount, matched: res.matchedCount });
    } catch (error) {
        console.error("Error marking easypay requests as duplicates:", (error as Error).message);
        return NextResponse.json(
            { message: "Internal Server Error ~ mark easypay duplicates" },
            { status: 500 }
        );
    }
}
