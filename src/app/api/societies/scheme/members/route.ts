...
export async function POST(request: Request) {
  try {
    ...
    const member = await SocietyMemberModel.create({
      ...memberDetails,
    ...
    return NextResponse.json(
      { success: true, message: "Society member created successfully", member },
      { status: 200 }
    );
  } catch (error: any) {
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Internal Server Error ~ Error creating society member",
      },
      { status: 500 }
    );
  }
