  ...
  try {
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        {
          folder: `cash-up/sales-receipts/${submissionIdentifier}`,
          resource_type: "auto",
        },
        (err, result) => {
          if (err) {
    ...
    console.log("🚀 ~ POST ~ sales uploadResult to cash up submission:", uploadResult)
    ...
    console.error("Sales receipt upload failed to cash up submission:", err);
    return NextResponse.json(
