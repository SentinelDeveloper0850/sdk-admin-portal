  ...
  try {
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        {
          folder: `daily-audit/funeral-receipts/${submissionIdentifier}`,
        },
        (err, result) => {
          if (err) {
    ...
    console.log("🚀 ~ POST ~ funeral uploadResult:", uploadResult)
    ...
    console.error("Funeral receipt upload failed:", err);
    return NextResponse.json(
