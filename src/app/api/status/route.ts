...
import { AnnouncementAckModel, AnnouncementModel, AnnouncementReadModel } from "@/app/models/system/announcement.schema";
...
export async function GET(request: NextRequest) {
    try {
        ...
        const [branchCount, userCount, policyCount, funeralCount, calendarEventCount, notificationCount, configurationCount, auditLogCount, announcementCount, announcementReadCount, announcementAckCount, eftTransactionCount, easypayTransactionCount, allocationRequestCount, policySignUpCount, policyCancellationRequestCount, claimCount, dailyActivityCount, schemeSocietyCount] = await Promise.all([
        ...
        const status = {
            ...
            models: {
                ...
                announcementRead: { count: announcementReadCount, name: "Announcement Read" },
                announcementAck: { count: announcementAckCount, name: "Announcement Ack" },
                eftTransaction: { count: eftTransactionCount, name: "EFT Transaction" },
                easypayTransaction: { count: easypayTransactionCount, name: "EasyPay Transaction" },
                allocationRequest: { count: allocationRequestCount, name: "Allocation Request" },
                policySignUp: { count: policySignUpCount, name: "Policy Sign Up" },
                policyCancellationRequest: { count: policyCancellationRequestCount, name: "Policy Cancellation Request" },
