...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !code) {
      return NextResponse.json(
        { success: false, error: { message: "Missing required fields: name, code" } },
        { status: 400 }
      )
    }
    ...
    if (!regionId) {
      return NextResponse.json(
        { success: false, error: { message: "Missing required field: regionId" } },
        { status: 400 }
      )
    }
    ...
    const region = await RegionModel.findOne({ id: String(regionId).trim(), isActive: true })
    if (!region) {
      return NextResponse.json(
    ...
    if (existingByCode) {
      return NextResponse.json(
        { success: false, error: { message: "Branch with this code already exists" } },
        { status: 409 }
      )
    }
    ...
    const payload = {
      ...
      phoneExtension: typeof phoneExtension === "string" ? phoneExtension.trim() : undefined,
      email: emailValue,
    ...
    const hydrated = await BranchModel.findById(savedBranch._id).populate("regionDoc").populate("manager")
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { success: false, error: { message: "Branch business id is required" } },
        { status: 400 }
      )
    }
    ...
    const result = await BranchModel.findOneAndDelete({ id: String(id).trim() })
