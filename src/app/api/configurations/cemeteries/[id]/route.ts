...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (body?.province !== undefined) patch.province = String(body.province || "");
        if (body?.postalCode !== undefined) patch.postalCode = String(body.postalCode || "");
        if (body?.latitude !== undefined) patch.latitude = body.latitude ?? undefined;
        if (body?.longitude !== undefined) patch.longitude = body.longitude ?? undefined;
        ...
        const updated = await CemeteryModel.findByIdAndUpdate(id, patch, { new: true });
