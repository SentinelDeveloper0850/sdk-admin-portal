...
export async function POST(request: NextRequest) {
    try {
        ...
        if (existingId) {
            return NextResponse.json({ success: false, error: "Region ID already exists" }, { status: 409 })
        }
        ...
        return NextResponse.json({ success: true, data: created, message: "Region created successfully" })
    } catch (error) {
