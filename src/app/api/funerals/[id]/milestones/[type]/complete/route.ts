...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; type: string } }) {
    try {
        ...
        const completedAt = body?.completedAt ? dayjs(body.completedAt).toDate() : new Date();
        ...
        if (!ms) return NextResponse.json({ success: false, message: "Milestone not found" }, { status: 404 });
        ...
        await upsertFuneralCalendarEvents(funeral, { id: String(user._id), name: user.name || user.email });

        return NextResponse.json({ success: true, funeral }, { status: 200 });
    } catch (err) {
        console.error("PATCH milestone complete error:", err);
        return NextResponse.json({ success: false, message: "Failed to complete milestone" }, { status: 500 });
    }
}
