...
import {
  ...
  TASK_STATUSES,
  TaskModel,
} from "@/app/models/system/task.schema";
...
export async function POST(req: NextRequest) {
  ...
  const priority = TASK_PRIORITIES.includes(priorityRaw as any)
    ? priorityRaw
    : "MEDIUM";

  const dueAt = body?.dueAt ? new Date(body.dueAt) : undefined;
  const safeDueAt =
    dueAt instanceof Date && !Number.isNaN(dueAt.getTime()) ? dueAt : undefined;
