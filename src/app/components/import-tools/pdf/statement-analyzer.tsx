...
export default function PDFBankStatementAnalyzer() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!pdfText || pdfText.trim() === "") {
        throw new Error(
          "No text could be extracted from the PDF. The file might be empty or protected."
        );
      }

      // Convert the extracted text to CSV format
      addToLog(
        `Converting text to CSV format (skipping first ${skipLines} lines)...`
      );
  ...
  const convertTextToStructuredCSV = (text: string, skipLines = 0): string => {
    ...
    for (let i = 0; i < processedLines.length; i++) {
      ...
      const amountMatches =
        line.match(new RegExp(amountPattern.source, "g")) || [];
    ...
    for (const transaction of potentialTransactions) {
      ...
      const lineAfterAmount = line
        .substring(line.indexOf(amount) + amount.length)
        .trim();
  ...
  const convertTextToStructuredCSVwithOnlyCredits = (
    text: string,
    skipLines = 0
  ): string => {
    const lines = text.split("\n").filter((line) => line.trim() !== "");
    ...
    for (const line of processedLines) {
      ...
      const afterAmount = line
        .substring(secondLastAmount.index! + secondLastAmount[0].length)
        .trim();
        ...
        <TabsContent value="upload">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload your PDF file to extract its content and convert it to
                CSV format.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col items-center justify-center py-10">
              {isLoading ? (
                ...
                <div
                  className="w-full max-w-md rounded-lg border-2 border-dashed border-gray-300 p-12 text-center"
                ...
                >
                  ...
                  <Label htmlFor="file-upload" className="cursor-pointer">
                    <div className="inline-flex items-center rounded-md bg-primary px-4 py-2 text-primary-foreground hover:bg-primary/90">
              ...
              <div className="mt-4 flex w-full max-w-md flex-col gap-2">
                <div className="flex items-center">
                  ...
                  <Label htmlFor="preserveStructure" className="ml-2 text-sm">
                    Try to preserve bank statement structure (recommended for
                    bank statements)
                  </Label>
                </div>

                <div className="mt-2 flex items-center">
                  <Label htmlFor="skipLines" className="mr-2 w-40 text-sm">
                    Skip first lines:
                  </Label>
                  type
                  <Input
        ...
        <TabsContent value="raw">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The raw text extracted from your PDF and the converted CSV
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="log">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Detailed log of the PDF processing steps
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="bg-muted overflow-x-auto rounded-md p-4">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                This tool extracts text from PDF files and converts it to CSV
                format. Here are some tips for best results:
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              <ul className="list-disc space-y-1 pl-5 text-sm">
                ...
                <li>
                  The table view automatically filters out rows that don't
                  follow the "date, description, balance" structure
                </li>
              </ul>
            </div>
            ...
            <div className="space-y-2">
              ...
              <ul className="list-disc space-y-1 pl-5 text-sm">
                <li>
                  If the CSV format is not ideal, try the "Alternative Format"
                  button
                </li>
                <li>
                  Some PDFs may have complex layouts that don't convert well to
                  CSV
                </li>
                <li>
                  Check the Processing Log tab for detailed information about
                  any errors
                </li>
              </ul>
            </div>
