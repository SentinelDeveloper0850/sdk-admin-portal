...
function milestoneFromRecord(
  record: IFuneral | null | undefined,
  key: MilestoneKey
) {
  if (!record) return undefined;
  const type = KEY_TO_TYPE[key];
  const list = Array.isArray((record as any).milestones)
    ? (record as any).milestones
    : [];
...
export default function CaseFileSummary({
    ...
    if (usingForm) {
      switch (String(path)) {
        case "referenceNumber":
          return w_referenceNumber ?? fallback;
        case "policyNumber":
          return w_policyNumber ?? fallback;
        case "branchId":
          return w_branchId ?? fallback;
        case "deceasedFirstName":
          return w_deceasedFirstName ?? fallback;
        case "deceasedLastName":
          return w_deceasedLastName ?? fallback;
        case "informantFirstName":
          return w_informantFirstName ?? fallback;
        case "informantLastName":
          return w_informantLastName ?? fallback;
        case "informantPhoneNumber":
          return w_informantPhoneNumber ?? fallback;
        case "status":
          return w_status ?? fallback;
        case "paymentStatus":
          return w_paymentStatus ?? fallback;
        case "estimatedCost":
          return w_estimatedCost ?? fallback;
        case "actualCost":
          return w_actualCost ?? fallback;
        default:
          return fallback;
      }
    }
  ...
  const deceasedFirstName = record
    ? record.deceased?.firstName
    : get("deceasedFirstName");
  const deceasedLastName = record
    ? record.deceased?.lastName
    : get("deceasedLastName");

  const informantFirstName = record
    ? record.informant?.firstName
    : get("informantFirstName");
  const informantLastName = record
    ? record.informant?.lastName
    : get("informantLastName");
  const informantPhone = record
    ? record.informant?.phoneNumber
    : get("informantPhoneNumber");
  ...
  const deceasedName =
    `${deceasedFirstName ?? ""} ${deceasedLastName ?? ""}`.trim() || "—";
  const informantName =
    `${informantFirstName ?? ""} ${informantLastName ?? ""}`.trim() || "—";
  ...
  const enabledWithDates = MILESTONES.map((m) => {
  ...
  const cemeteryName = cemeteryCode
    ? (cemeteryMap?.[cemeteryCode] ?? cemeteryCode)
    : "—";
  ...
  for (const m of MILESTONES) {
    const v = milestoneValues[m.key] || {};
    if (v?.enabled && !v?.startDateTime)
      warnings.push({
        text: `${m.label} enabled but no date/time`,
        jumpKey: `m_${m.key}`,
      });
  }
  ...
  if (escort?.enabled) {
    if (!escort?.origin)
      warnings.push({
        text: "Escort enabled but origin missing",
        jumpKey: "m_serviceEscort",
      });
    if (!escort?.destination)
      warnings.push({
        text: "Escort enabled but destination missing",
        jumpKey: "m_serviceEscort",
      });
  }
  ...
  return (
    <>
      ...
      <Divider className="!my-3 !text-xs !uppercase !italic">
        Burial Details
      </Divider>
      ...
      <div className="space-y-2">
        ...
        {typeof delta === "number" ? (
          <div className="flex items-center justify-between gap-2">
            ...
            <Text
              type={delta > 0 ? "danger" : delta < 0 ? "success" : undefined}
            >
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Text>
            {next?.startDateTime ? (
              <>
                {fmtDT(next.startDateTime)}{" "}
                <Text type="secondary">{rel(next.startDateTime)}</Text>
              </>
            ) : (
              "—"
            )}
              ...
              >
                ...
                <Space size={8}>
                  <Text type="secondary">
                    ...
                    {enabled && dt && burialCemeteryInline
                      ? ` • ${burialCemeteryInline}`
                      : ""}
                  </Text>
      ...
      {warnings.length ? (
        <>
          <Divider className="!my-3 !text-xs !uppercase !italic !text-red-500">
            Warnings
          </Divider>
          <div className="space-y-1">
