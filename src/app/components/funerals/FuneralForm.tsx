...
const notesTextToNotesArray = (text: any) => {
  ...
  if (!s) return [];
  return [
    { note: s, createdAt: new Date(), createdBy: "UI", createdById: "UI" },
  ];
};
...
const buildMilestonesFromForm = (v: any) => {
  ...
  for (const def of MILESTONE_FORM) {
    ...
    const endISO =
      slot.endDateTime && dayjs.isDayjs(slot.endDateTime)
      ...
      <Popconfirm
        ...
        onConfirm={async () => {
          const res = await fetch(
            `/api/funerals/${funeralId}/milestones/${def.type}/complete`,
            {
              method: "PATCH",
          ...
          if (!res.ok || !json?.success) {
            sweetAlert({
              title: "Error",
              text: json?.message || "Failed to mark completed",
              icon: "error",
            });
            return;
          }
  ...
  useEffect(() => {
    const loadCemeteries = async () => {
      try {
        ...
        sweetAlert({
          title: "Error",
          text: "Failed to load cemeteries",
          icon: "error",
        });
      } finally {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const json = await res.json();
        if (!res.ok || !json?.success)
          throw new Error(json?.message || "Failed to load funeral");
        ...
        form.setFieldsValue({
          ...
          deceasedDateOfBirth: row.deceased?.dateOfBirth
            ? dayjs(row.deceased.dateOfBirth)
            : undefined,
          deceasedDateOfDeath: row.deceased?.dateOfDeath
            ? dayjs(row.deceased.dateOfDeath)
            : undefined,
        ...
        sweetAlert({
          title: "Error",
          text: e?.message || "Failed to load funeral",
          icon: "error",
        });
      } finally {
        setLoading(false);
    ...
    try {
      ...
      const body: any = {
        ...
        deceased: {
          ...
          dateOfBirth: v.deceasedDateOfBirth
            ? v.deceasedDateOfBirth.toISOString()
            : undefined,
          dateOfDeath: v.deceasedDateOfDeath
            ? v.deceasedDateOfDeath.toISOString()
            : undefined,
        },
      ...
      const url =
        mode === "edit" ? `/api/funerals/${funeralId}` : "/api/funerals";
    ...
    <Form form={form} layout="vertical">
      <div className="grid grid-cols-12 items-start gap-4">
          ...
          >
                        ...
                        validator: async (_, value: string) => {
                          ...
                          if (first === "0")
                            return Promise.reject(
                              new Error("First digit cannot be 0")
                            );
                      ...
                      },
                      {
                        validator: async (_, value: string) => {
                          ...
                          if (isNaN(Number(digits)))
                            return Promise.reject(
                              new Error("Phone number must be numbers")
                            );
                          if (digits.length !== 9)
                            return Promise.reject(
                              new Error("Phone number must be 9 digits")
                            );
                          return Promise.resolve();
                        },
                      },
            ...
            {MILESTONE_FORM.map((def, idx) => (
              <Collapse.Panel
                key={`m_${def.key}`}
                header={
                  <h4 className="!text-xs !font-bold uppercase !tracking-wider">{`${4 + idx}. ${def.label}`}</h4>
                }
              >
                <div id={`panel_m_${def.key}`} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <Form.Item
                      name={[def.key, "enabled"]}
                  ...
                  {def.type === "delivery" ? (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Form.Item
                        ...
                        rules={[
                          { required: true, message: "Cemetery is required" },
                        ]}
                      >
                        <Select
                          showSearch
            ...
            <Collapse.Panel
              header={
                <h4 className="!text-xs !font-bold uppercase !tracking-wider">{`${4 + MILESTONE_FORM.length}. Final Notes`}</h4>
              }
              key="notes"
            >
              <div id="panel_notes">
                <Form.Item label="Funeral Notes" name="notes">
                  <Input.TextArea
                    rows={4}
                    placeholder="Internal notes (saved as Note[] with placeholder audit fields)"
                  />
                </Form.Item>
              </div>
            </Collapse.Panel>
        ...
        <div className="col-span-12 xl:col-span-4">
          <div className="sticky top-4 rounded-md bg-white p-4 shadow-sm">
