        ...
        <Popover open={open} onOpenChange={(isOpen) => {
            ...
            >
                ...
                <ScrollArea className="h-80">
                    ...
                    {isError && !isLoading && (
                        <div className="px-3 py-4 text-xs text-destructive">Failed to load tasks.</div>
                    )}

                    {!isLoading && !isError && tasks.length === 0 && (
                        <div className="px-3 py-4 text-xs text-muted-foreground">
                            No open tasks. Either you’re a machine… or someone forgot to assign you work.
                        </div>
                    )}
                    ...
                    <ul className="flex flex-col gap-1 p-2">
                        {tasks.map((t) => {
                            ...
                            const unseen = !t.seenAt || new Date(t.updatedAt).getTime() > new Date(t.seenAt).getTime();
                            return (
                                <li
                                    key={t._id}
                                    className={cn(
                                ...
                                >
                                    <div className="flex items-center justify-between gap-2">
                                        <span className={cn("font-medium text-xs", overdue && "text-destructive")}>
                                            {t.title}
                                        </span>
