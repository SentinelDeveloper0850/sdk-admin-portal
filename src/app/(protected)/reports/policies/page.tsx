...
export default function ReportsPage() {
  ...
  const fetchReportData = async () => {
    try {
      ...
      const easyPayData = easyPayResponse.ok
        ? await easyPayResponse.json()
        : null;
...
- Low: Review file-only and database-only policies
  ...
  const exportToPDF = () => {
    ...
    const generatePDFContent = (startY: number) => {
      ...
      doc.text("RECONCILIATION REPORT", pageWidth / 2, currentY, {
        align: "center",
      });
      currentY += 15;
      ...
      currentY += lineHeight;
      doc.text(
        `- Valid records: ${policyData.fileData.valid}`,
        margin + 5,
        currentY
      );
      currentY += lineHeight;
      doc.text(
        `- Invalid records: ${policyData.fileData.invalid}`,
        margin + 5,
        currentY
      );
      ...
      doc.text(
        `Database Records: ${policyData.databaseData.total}`,
        margin,
        currentY
      );
      currentY += lineHeight;
      doc.text(
        `- With EasyPay numbers: ${policyData.databaseData.withEasyPayNumber}`,
        margin + 5,
        currentY
      );
      currentY += lineHeight;
      doc.text(
        `- Without EasyPay numbers: ${policyData.databaseData.withoutEasyPayNumber}`,
        margin + 5,
        currentY
      );
      currentY += lineHeight + 5;
      ...
      currentY += lineHeight;
      doc.text(
        `Database Only: ${policyData.databaseOnly.length}`,
        margin,
        currentY
      );
      currentY += lineHeight;
      ...
      currentY += lineHeight;
      doc.text(
        `Without EasyPay: ${policyData.withoutEasyPay.length}`,
        margin,
        currentY
      );
      ...
      currentY += lineHeight;
      doc.text(
        `Unique EasyPay Numbers without Policy Numbers: ${easyPayData.uniqueEasyPayWithoutPolicy}`,
        margin,
        currentY
      );
      currentY += lineHeight;
      doc.text(
        `Transactions without Policy Numbers: ${easyPayData.withoutPolicy}`,
        margin,
        currentY
      );
      currentY += lineHeight + 10;
      ...
      const totalSectionHeight =
        headerHeight + stepsHeight + priorityHeight + 20; // +20 for spacing
        ...
        <>
          ...
          >
            <Row gutter={[16, 16]}>
              <Col span={8}>
                <Statistic
                  title={
                    <span className="dark:text-gray-300">File Records</span>
                  }
                ...
                <div style={{ marginTop: "8px" }}>
                  <Tag color="success">
                    Valid: {reportData.policyReconciliation.fileData.valid}
                  </Tag>
                  <Tag color="error">
                    Invalid: {reportData.policyReconciliation.fileData.invalid}
                  </Tag>
                </div>
              </Col>
              <Col span={8}>
                <Statistic
                  title={
                    <span className="dark:text-gray-300">Database Records</span>
                  }
                ...
                <div style={{ marginTop: "8px" }}>
                  <Tag color="success">
                    With EasyPay:{" "}
                    {
                      reportData.policyReconciliation.databaseData
                        .withEasyPayNumber
                    }
                  </Tag>
                  <Tag color="warning">
              ...
              <Col span={8}>
                ...
                />
                <Text className="dark:text-gray-400" type="secondary">
                  Policy numbers that exist in both file and database with
                  matching EasyPay numbers
                </Text>
              </Col>
            </Row>
            ...
            <Row gutter={[16, 16]}>
              <Col span={6}>
                <Statistic
                  ...
                  valueStyle={{
                    color:
                      getPriorityLevel(
                        reportData.policyReconciliation.fileOnly.length
                      ) === "error"
                        ? "#ff4d4f"
                        : "#faad14",
                  }}
              ...
              <Col span={6}>
                <Statistic
                  title={
                    <span className="dark:text-gray-300">Database Only</span>
                  }
                  value={reportData.policyReconciliation.databaseOnly.length}
                  valueStyle={{
                    color:
                      getPriorityLevel(
                        reportData.policyReconciliation.databaseOnly.length
                      ) === "error"
                        ? "#ff4d4f"
                        : "#faad14",
                  }}
              ...
              <Col span={6}>
                <Statistic
                  ...
                  valueStyle={{
                    color:
                      getPriorityLevel(
                        reportData.policyReconciliation.mismatches.length
                      ) === "error"
                        ? "#ff4d4f"
                        : "#faad14",
                  }}
                />
                <Text className="dark:text-gray-400" type="secondary">
                  Policy numbers with different EasyPay numbers in file vs
                  database
                </Text>
                <Tag
                  color={getPriorityLevel(
              ...
              <Col span={6}>
                <Statistic
                  title={
                    <span className="dark:text-gray-300">Without EasyPay</span>
                  }
                  value={reportData.policyReconciliation.withoutEasyPay.length}
                  valueStyle={{
                    color:
                      getPriorityLevel(
                        reportData.policyReconciliation.withoutEasyPay.length
                      ) === "error"
                        ? "#ff4d4f"
                        : "#faad14",
                  }}
            ...
            <Row gutter={[16, 16]}>
              ...
              <Col span={8}>
                <Statistic
                  title={
                    <span className="dark:text-gray-300">
                      Unique EasyPay Numbers without Policy Numbers
                    </span>
                  }
                  ...
                  valueStyle={{
                    color:
                      getPriorityLevel(
                        reportData.easyPayStats.uniqueEasyPayWithoutPolicy
                      ) === "error"
                        ? "#ff4d4f"
                        : "#faad14",
                  }}
                />
                <Text className="dark:text-gray-400" type="secondary">
                  Number of unique EasyPay numbers that don't have corresponding
                  policy numbers
                </Text>
                <Tag
                  color={getPriorityLevel(
              ...
              <Col span={8}>
                <Statistic
                  title={
                    <span className="dark:text-gray-300">
                      Transactions without Policy Numbers
                    </span>
                  }
                  ...
                  valueStyle={{
                    color:
                      getPriorityLevel(
                        reportData.easyPayStats.withoutPolicy
                      ) === "error"
                        ? "#ff4d4f"
                        : "#faad14",
                  }}
                />
                <Text className="dark:text-gray-400" type="secondary">
                  Total number of individual transactions that don't have policy
                  numbers assigned
                </Text>
                <Tag
                  color={getPriorityLevel(
          ...
          >
            <Alert
              ...
              type="info"
              showIcon
              className="dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200"
