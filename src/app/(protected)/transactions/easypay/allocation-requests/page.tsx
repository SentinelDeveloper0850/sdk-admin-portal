...
interface EasypayAllocationRequestItem {
    _id: string;
...
export default function EasypayAllocationRequestsPage() {
    ...
    const [items, setItems] = useState<EasypayAllocationRequestItem[]>([]);
    const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
    const [filters, setFilters] = useState<{ status?: string; start?: string; end?: string; requester?: string }>({});

    const [rejecting, setRejecting] = useState<EasypayAllocationRequestItem | null>(null);
    const [rejectForm] = Form.useForm();
    const [reviewing, setReviewing] = useState<EasypayAllocationRequestItem | null>(null);
    const [reviewLoading, setReviewLoading] = useState(false);
    const [reviewDetail, setReviewDetail] = useState<{ item: EasypayAllocationRequestItem; transaction: EasypayTransactionDetail } | null>(null);
    ...
    const generateAndDownloadCSV = (requests: any[], filename: string) => {
        ...
        const csvRows = requests.map(request => {
            const membershipNo = request.request.easypayNumber;
            ...
            for (const { request, transaction } of requestsWithTransactions) {
                ...
                const matches = csvData.filter(csvRow => {
                    ...
                    return effectiveDate === csvFormattedTransactionDate &&
                        membershipId.toString().trim() === request.easypayNumber.toString().trim();
                });
        ...
        <div style={{ padding: "20px" }}>
            ...
            <Table
                ...
                rowSelection={(filters.status === 'APPROVED' || (isAllocator && filters.status === 'SUBMITTED')) ? {
                    ...
                    getCheckboxProps: (record: EasypayAllocationRequestItem) => ({ disabled: !((!isAllocator && record.status === 'APPROVED') || (isAllocator && record.status === 'SUBMITTED')) }),
                } : undefined}
                dataSource={items}
                columns={[
                    {
                    ...
                    },
                    {
                        ...
                        key: "easypayNumber",
                        sorter: (a: any, b: any) => String(a.easypayNumber).localeCompare(String(b.easypayNumber)),
                    },
                    {
                        title: "Policy Number",
                        dataIndex: "policyNumber",
                        key: "policyNumber",
                        render: (policyNumber: string) => policyNumber || "—",
                    },
                    {
                        title: "Requested By",
                        key: "requestedBy",
                        render: (_: any, r: EasypayAllocationRequestItem) => {
                    ...
                    },
                    {
                        ...
                        key: "evidence",
                        render: (_: any, record: EasypayAllocationRequestItem) => record.evidence?.length || 0,
                    },
                    {
                        title: "Status",
                        dataIndex: "status",
                    ...
                    },
                    {
                        title: "Actions",
                        key: "actions",
                        render: (_: any, record: EasypayAllocationRequestItem) => (
                            <Dropdown
                                menu={{
                                    items: [
                                        {
                ...
                {!reviewLoading && reviewDetail && (
                    <div>
                        ...
                        <Descriptions size="small" bordered column={2} style={{ marginBottom: 16 }}>
                            ...
                            {reviewDetail.transaction.policyNumber && (
                                <Descriptions.Item label="Policy Number">{reviewDetail.transaction.policyNumber}</Descriptions.Item>
                            )}
                        </Descriptions>

                        <h3 className="mb-2 text-md font-semibold">Request Information</h3>
                        <Descriptions size="small" bordered column={2} style={{ marginBottom: 16 }}>
                            <Descriptions.Item label="Easypay Number">{reviewDetail.item.easypayNumber}</Descriptions.Item>
                            <Descriptions.Item label="Requested On">{new Date(reviewDetail.item.createdAt).toLocaleString()}</Descriptions.Item>
                            <Descriptions.Item label="Status"><Tag color={reviewDetail.item.status === 'PENDING' ? 'gold' : reviewDetail.item.status === 'APPROVED' ? 'green' : reviewDetail.item.status === 'REJECTED' ? 'red' : reviewDetail.item.status === 'DUPLICATE' ? 'orange' : 'blue'}>{reviewDetail.item.status}</Tag></Descriptions.Item>
                            ...
                            {reviewDetail.item.status === 'DUPLICATE' && (
                                <>
                                    <Descriptions.Item label="Marked as Duplicate By">{(reviewDetail.item as any).markedAsDuplicateBy?.name || (reviewDetail.item as any).markedAsDuplicateBy?.email || '—'}</Descriptions.Item>
                                    <Descriptions.Item label="Marked as Duplicate At">{(reviewDetail.item as any).markedAsDuplicateAt ? new Date((reviewDetail.item as any).markedAsDuplicateAt).toLocaleString() : '—'}</Descriptions.Item>
                                </>
                            )}
            ...
            {/* Scan Duplicates Drawer - Similar to EFT implementation */}
            <Drawer
                title={
                    <div>
                                                            ...
                                                            render: (_, record, index) => (
                                                                <Space>
                                                                    <Button
                                                                        ...
                                                                        onClick={() => {
                                                                            const timestamp = dayjs().format('YYYY-MM-DD_HH-mm-ss');
                                                                            const filename = `easypay_allocation_request_${record.request.easypayNumber}_${timestamp}.csv`;
                                                                            generateAndDownloadCSV([record], filename);
                                                                            message.success(`Downloaded request for easypay number ${record.request.easypayNumber}`);
                                                                        }}
                                                                    >
                                                                        Download
                                                                    </Button>
                                                                    <Button
                                                                        ...
                                                                        onClick={async () => {
                                                                            ...
                                                                            if (res.ok) {
                                                                                handleRefresh();
                                                                                message.success(`Marked easypay number ${record.request.easypayNumber} as allocated`);
                                                                            } else {
                                                                                const data = await res.json().catch(() => ({}));
                                                                ...
                                                                <Button
                                                                    ...
                                                                    onClick={async () => {
                                                                        try {
                                                                            ...
                                                                            if (res.ok) {
                                                                                handleRefresh();
                                                                                message.success(`Marked easypay number ${record.request.easypayNumber} as duplicate`);
                                                                            } else {
                                                                                const data = await res.json().catch(() => ({}));
