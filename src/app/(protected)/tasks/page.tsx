...
type TaskStatus = "BACKLOG" | "TODO" | "IN_PROGRESS" | "BLOCKED" | "DONE" | "CANCELED";
...
export default function TasksPage() {
    ...
    const handleCreate = async (values: any) => {
        try {
            ...
            if (res.status !== 201) {
                sweetAlert({ title: "Failed to create task!", text: res.data?.error, icon: "error" });
                return;
            }
    ...
    const handleUpdate = async (values: any) => {
        ...
        try {
            ...
            const res = await axios.patch(`/api/tasks/${editingTask._id}`, JSON.stringify(payload), {
                headers: { "Content-Type": "application/json" },
                withCredentials: true,
            });

            if (res.status !== 200) {
                sweetAlert({ title: "Failed to update task!", text: res.data?.error, icon: "error" });
                return;
            }
    ...
    const toggleDone = async (record: Task) => {
        try {
            ...
            if (!res.ok) {
                sweetAlert({ title: "Failed", text: data?.error || "Could not update task", icon: "error" });
                return;
            }

            setTasks((prev) => prev.map((t) => (t._id === record._id ? data.task : t)));
            sweetAlert({ title: goingDone ? "Marked as done" : "Reopened", icon: "success", timer: 1500 });
        } catch (e) {
            console.error(e);
            sweetAlert({ title: "Error", text: "Could not update task", icon: "error" });
        } finally {
            setLoading(false);
        }
    };
    ...
    const columns = useMemo(() => {
        return [
                ...
                render: (value: string, record: Task) => (
                    <div className="flex flex-col w-full">
                        <div className="flex items-center gap-2">
                            <span className={isOverdue(record) ? "font-semibold text-red-600" : "font-medium"}>
                                {value}
                            </span>
                    ...
                    return (
                        <div className="flex items-center gap-4">
                            {u?.avatarUrl && <Avatar
                                ...
                                radius="full"
                            />}
                            <span className="text-sm">{u?.name ?? u?.email ?? "Unknown"}</span>
                        </div>
                    );
                },
                sorter: (a: Task, b: Task) => String(a.assigneeUserId || "").localeCompare(String(b.assigneeUserId || "")),
            },
            {
                title: "Status",
                dataIndex: "status",
            ...
            },
            {
                ...
                dataIndex: "priority",
                key: "priority",
                render: (value: TaskPriority) => <Tag color={priorityTagColor(value)}>{value}</Tag>,
            ...
            },
            {
                ...
                render: (value: string, record: Task) =>
                    value ? (
                        <span className={isOverdue(record) ? "text-red-600 font-semibold" : ""}>
                            {dayjs(value).fromNow()}
                        </span>
                    ) : (
            ...
            },
            {
                ...
                dataIndex: "updatedAt",
                key: "updatedAt",
                render: (value: string) => <span>{value ? dayjs(value).fromNow() : "-"}</span>,
                sorter: (a: Task, b: Task) =>
            ...
            },
            {
                ...
                render: (_: any, record: Task) => (
                    <Dropdown
                        ...
                        menu={{
                            items: [
                                ...
                                {
                                    key: "toggle-done",
                                    icon: record.status === "DONE" ? <RollbackOutlined /> : <CheckCircleOutlined />,
                                    label: record.status === "DONE" ? "Reopen (To Do)" : "Mark Done",
                                    onClick: (e) => {
                                        e.domEvent?.stopPropagation?.();
                                        toggleDone(record);
                                    },
                    ...
                    >
                        <AntButton icon={<MoreOutlined />} onClick={(e) => e.stopPropagation()} />
                    </Dropdown>
                ),
            },
        ];
    ...
    const userOptions = useMemo(() => {
        return (users || []).map((u: any) => ({
            label: u?.name ? `${u.name} (${u.email ?? "no email"})` : (u.email ?? "Unknown"),
            value: u._id ?? u.id,
        }));
    }, [users]);
        ...
        <div style={{ padding: "20px" }}>
                ...
                actions={[
                    <div className="flex items-center gap-4" key="actions">
                        <div className="flex items-center gap-2">
                            ...
                            <Switch size="sm" isSelected={viewType === "compact"} onValueChange={() => toggleViewType()} />
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-sm">Show Archived</span>
                            <Switch size="sm" isSelected={showArchived} onValueChange={setShowArchived} />
                        </div>
                        ...
                        <AntButton icon={<ReloadOutlined />} loading={loading} onClick={fetchTasks}>
                            Refresh
                        </AntButton>
            ...
            <Table
                ...
                expandable={{
                    ...
                    rowExpandable: (record: Task) => !!record.tags && record.tags.length > 0,
                }}
            />
                ...
                <Form form={form} layout="vertical" onFinish={handleUpdate}>
                    ...
                    {editingTask && (
                        <div className="mt-4 text-xs text-gray-500">
                            <div>
                                <strong>Created:</strong> {dayjs(editingTask.createdAt).fromNow()}
                            </div>
                            <div>
                                <strong>Last updated:</strong> {dayjs(editingTask.updatedAt).fromNow()}
                            </div>
                            {editingTask.completedAt ? (
                                <div>
                                    <strong>Completed:</strong> {dayjs(editingTask.completedAt).fromNow()}
                                </div>
                            ) : null}
                        </div>
                    )}
