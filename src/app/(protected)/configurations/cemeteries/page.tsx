...
const CemeteriesConfigurationsPage = () => {
  ...
  const [editingCemetery, setEditingCemetery] = useState<CemeteryConfig | null>(
    null
  );
  const [originalCemeteryData, setOriginalCemeteryData] =
    useState<Partial<CemeteryConfig> | null>(null);
  ...
  const handleSaveCemetery = async (values: any) => {
    try {
      ...
      if (json?.success) {
        sweetAlert({
          title: "Success",
          text: "Cemetery saved successfully",
          icon: "success",
        });
        form.resetFields();
        ...
        sweetAlert({
          title: "Error",
          text: json?.error || json?.message || "Failed to save cemetery",
          icon: "error",
        });
      }
    } catch (e) {
      ...
      sweetAlert({
        title: "Error",
        text: "An error occurred while saving cemetery",
        icon: "error",
      });
    } finally {
      setSaving(false);
  ...
  const handleUpdateCemetery = async (values: any) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/configurations/cemeteries/${editingCemetery._id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (json?.success) {
        sweetAlert({
          title: "Success",
          text: "Cemetery updated successfully",
          icon: "success",
        });
        ...
        sweetAlert({
          title: "Error",
          text: json?.error || json?.message || "Failed to update cemetery",
          icon: "error",
        });
      }
    } catch (e) {
      ...
      sweetAlert({
        title: "Error",
        text: "An error occurred while updating cemetery",
        icon: "error",
      });
    } finally {
      setSaving(false);
  ...
  const handleDeleteCemetery = async (id: string) => {
    try {
      const res = await fetch(`/api/configurations/cemeteries/${id}`, {
        method: "DELETE",
      });
      const json = await res.json();
      ...
      if (json?.success) {
        sweetAlert({
          title: "Success",
          text: "Cemetery deleted successfully",
          icon: "success",
        });
        fetchCemeteries();
      } else {
        sweetAlert({
          title: "Error",
          text: json?.error || json?.message || "Failed to delete cemetery",
          icon: "error",
        });
      }
    } catch (e) {
      ...
      sweetAlert({
        title: "Error",
        text: "An error occurred while deleting cemetery",
        icon: "error",
      });
    }
  };
    ...
    <div style={{ padding: 20 }}>
      ...
      >
        <Form
          form={form}
          layout="vertical"
          onFinish={handleSaveCemetery}
          className="grid grid-cols-3 gap-4"
        >
          <Form.Item
            name="name"
            label="Cemetery Name"
            rules={[{ required: true, message: "Please enter cemetery name" }]}
          >
            <Input
              prefix={<MapPin size={16} className="mr-1" />}
              placeholder="e.g. West Park Cemetery"
              allowClear
            />
          </Form.Item>
