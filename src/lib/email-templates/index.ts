...
Handlebars.registerHelper('formatDate', function (date: string | Date, format: string = 'DD/MM/YYYY') {
  if (!date) return '';
...
export const loadTemplate = (templateName: string): HandlebarsTemplateDelegate => {
  ...
  try {
    ...
    const templatePath = path.join(process.cwd(), 'src', 'lib', 'email-templates', 'templates', `${templateName}.hbs`);
