...
export async function getExpectedUsersFromDutyRoster(params: {
  ...
  const roster = await DutyRosterModel.findOne({ date: { $gte: dayStart, $lt: dayEndExclusive } }).lean();
  ...
  if (roster && staffIds.length === 0) {
    return { expectedUsers: [] as any[], expectedSource: "duty_roster" as const };
  }
  ...
  return { expectedUsers: null as any[] | null, expectedSource: "all_active_users" as const };
}
